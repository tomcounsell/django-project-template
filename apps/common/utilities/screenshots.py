...
class ScreenshotService:
    ...
    def capture(
        ...
        if not PLAYWRIGHT_AVAILABLE:
            print("Error: Playwright not available. Install with 'uv add --dev playwright'")
            return ""
    ...
    def capture_with_browser_agent(
        ...
        if not BROWSER_USE_AVAILABLE:
            print("Error: browser-use not available. Install with 'uv add --dev browser-use'")
            return ""
        ...
        return asyncio.run(self._async_capture_with_agent(path, instructions, output_path))
...
if __name__ == "__main__":
    ...
    """
    parser = argparse.ArgumentParser(description="Capture screenshots of Django application pages")
    ...
    parser.add_argument("--wait-ms", type=int, default=500, help="Additional time to wait before capture (ms)")
    parser.add_argument("--full-page", action="store_true", help="Capture full page, not just viewport")
    parser.add_argument("--visible", action="store_true", help="Show browser window during capture")
    ...
    parser.add_argument("--use-agent", action="store_true", help="Use browser-use agent for capture")
    parser.add_argument("--instructions", help="Instructions for the browser-use agent")
    parser.add_argument("--ensure-server", action="store_true", help="Ensure a Django server is running")
