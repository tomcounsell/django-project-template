...
from apps.common.models import (
    User, Team, TeamMember, BlogPost, TodoItem, UserAPIKey, TeamAPIKey,
    Address, City, Country, Currency, Document, Email, SMS, Image, Note,
    Payment, Subscription, Upload
)

# Define which models should be shown in the main admin navigation
MAIN_NAV_MODELS = [
    'User', 'Team', 'BlogPost', 'TodoItem'
]
...
class UserAdmin(ModelAdmin):
    ...
    list_display = ['display_header', 'email', 'status_badge', 'display_staff', 'display_superuser', 'display_created']
    ...
    def display_header(self, instance):
        ...
        return [
            instance.username,
            None,
            f"{instance.first_name[:1]}{instance.last_name[:1]}".upper() if instance.first_name and instance.last_name else "",
        ]
...
class TeamMemberTableSection(TableSection):
    ...
    def role_display(self, instance):
        ...
        return format_html(
            ...
            color, instance.get_role_display()
        )
...
class TeamAdmin(ModelAdmin):
    list_display = ['display_header', 'slug', 'member_count', 'status_badge', 'created_at']
    list_filter = [('is_active', ChoicesDropdownFilter), ('created_at', RangeDateFilter)]
    list_filter_submit = True
    ...
    def display_header(self, instance):
        return [
            instance.name,
            instance.description[:30] + '...' if instance.description and len(instance.description) > 30 else instance.description,
        ]
    ...
    def export_teams(self, request):
        ...
        messages.success(request, 'Teams export initiated. You will receive an email when complete.')
...
class TodoItemAdmin(ModelAdmin):
    ...
    def display_header(self, instance):
        ...
        return [
            instance.title,
            instance.description[:50] + '...' if instance.description and len(instance.description) > 50 else instance.description,
        ]
    ...
    def mark_as_done(self, request):
        ...
        updated = TodoItem.objects.filter(id__in=selected).update(status='DONE', completed_at=now())
...
class AddressAdmin(ModelAdmin):
    ...
    fieldsets = (
        ("Address Information", {
            "fields": ("line_1", "line_2", "line_3", "city", "region", "postal_code", "country")
        }),
        ("Google Maps", {
    ...
    def map_link(self, obj):
        if obj.google_map_link:
            return format_html('<a href="{}" target="_blank" class="unfold-badge bg-blue-500 text-white">View Map</a>', obj.google_map_url)
        return "-"
...
class EmailAdmin(ModelAdmin):
    list_display = ('subject', 'to_address', 'from_address', 'email_type', 'sent_status', 'read_status')
...
class SMSAdmin(ModelAdmin):
    list_display = ('to_number', 'from_number', 'body_preview', 'delivery_status', 'sent_status')
    ...
    fieldsets = (
        ("SMS Details", {
            "fields": ("to_number", "from_number", "body")
        }),
        ("Twilio Information", {
...
class UploadAdmin(ModelAdmin):
    list_display = ('name_display', 'file_type_display', 'size_display', 'status_badge', 'created_at')
    list_filter = ('status', 'content_type')
    search_fields = ('name', 'original', 's3_key')
    readonly_fields = ('created_at', 'modified_at', 'size', 'dimensions_display', 'preview')
    
    fieldsets = (
        ("Upload Information", {
            "fields": ("name", "original", "thumbnail")
        }),
        ("File Information", {
    ...
    def preview(self, obj):
        ...
        if obj.is_image and obj.original:
            return format_html('<img src="{}" style="max-width: 300px; max-height: 200px;" />', obj.original)
        if obj.is_pdf and obj.original:
            return format_html('<a href="{}" target="_blank" class="unfold-badge bg-red-500 text-white">View PDF</a>', obj.original)
        return "-"
...
class ImageAdmin(ModelAdmin):
    ...
    fieldsets = (
        ("Image Information", {
            "fields": ("name", "original", "thumbnail_url")
        }),
        ("File Information", {
            "fields": ("dimensions_display", "preview")
        }),
        ("S3 Storage", {
            "fields": ("s3_bucket", "s3_key")
        }),
...
class PaymentAdmin(ModelAdmin):
    list_display = ('amount_display', 'status_badge', 'user', 'payment_method_display', 'subscription', 'paid_at')
    ...
    fieldsets = (
        ...
        ("Status", {
            "fields": ("status", "paid_at", "refunded_at")
        }),
        ("Subscription", {
            "fields": ("subscription",)
        }),
        ("Stripe", {
            "fields": ("stripe_payment_intent_id", "receipt_url")
        }),
        ("Timestamps", {
            "fields": ("created_at", "modified_at")
        }),
...
class SubscriptionAdmin(ModelAdmin):
    list_display = ('user', 'plan_name', 'price_display', 'status_badge', 'renewal_info', 'trial_badge')
    ...
    fieldsets = (
        ...
        ("Status", {
            "fields": ("status", "start_date", "end_date", "canceled_at")
        }),
        ("Billing Period", {
