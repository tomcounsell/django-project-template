...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('path', type=str, help='URL path to capture (e.g., /todos/)')
        parser.add_argument('--output-dir', default='screenshots', type=str, help='Directory to save screenshots')
        ...
        parser.add_argument('--visible', action='store_true', help='Show the browser during capture')
        parser.add_argument('--wait-for', type=str, help='CSS selector to wait for')
        parser.add_argument('--wait-ms', type=int, default=500, help='Time to wait before capture (ms)')
        parser.add_argument('--use-agent', action='store_true', help='Use AI agent for more complex scenarios')
    ...
    def handle(self, *args, **options):
        ...
        if screenshot_path:
            self.stdout.write(self.style.SUCCESS(f'Screenshot saved to: {screenshot_path}'))
        else:
