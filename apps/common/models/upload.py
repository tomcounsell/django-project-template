...
class Upload(Timestampable, models.Model):
  ...
  def width(self):
    if self.is_image:
      return self.meta_data['meta'].get('width') if self.meta_data.get('meta') else None

  @property
  def height(self):
    if self.is_image:
      return self.meta_data['meta'].get('height') if self.meta_data.get('meta') else None
  ...
  def link_title(self):
    if self.name:
      ...
      title = (self.meta_data['type'] or "").upper() if 'type' in self.meta_data else ""
