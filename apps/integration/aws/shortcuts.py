    ...
    try:
        ...
        if status == Upload.STATUS_COMPLETE and upload.s3_bucket and upload.s3_key:
            try:
                ...
                logger.error(f"Error updating metadata for upload {upload_id}: {str(e)}")
...
def get_upload_file_url(upload_id: int, presigned: bool = False, expiration: int = 3600) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        url = upload.get_presigned_url(expiration=expiration) if presigned else upload.s3_url
...
def delete_upload(upload_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        s3_deleted = upload.delete_from_s3() if upload.s3_bucket and upload.s3_key else False
        upload.delete()
