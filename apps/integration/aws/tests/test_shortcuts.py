...
class AWSShortcutsTestCase(TestCase):
    ...
    def test_get_direct_upload_form_data(self, mock_generate_filename, mock_s3_client_cls):
    ...
    def test_delete_upload(self):
        ...
        with mock.patch.object(Upload, 'delete_from_s3') as mock_delete_from_s3:
            ...
            self.assertFalse(result["success"])
            self.assertEqual(Upload.objects.filter(id=self.pending_upload.id).count(), 1)