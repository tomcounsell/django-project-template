        ...
        if mode == "payment":
            ...
            if payment_intent:
                ...
                payment = Payment.objects.create(
                    stripe_id=payment_intent if isinstance(payment_intent, str) else payment_intent.get("id"),
...
def handle_subscription_created(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if current_period_start:
            current_period_start = datetime.fromtimestamp(current_period_start, tz=timezone.get_current_timezone())
        
        if current_period_end:
            current_period_end = datetime.fromtimestamp(current_period_end, tz=timezone.get_current_timezone())
        ...
        logger.info(f"Created subscription record: {subscription.id}, Stripe ID: {subscription_id}")
...
def handle_subscription_updated(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if canceled_at:
            canceled_at = datetime.fromtimestamp(canceled_at, tz=timezone.get_current_timezone())
    ...
    try:
        ...
        if subscription_id:
            try:
                ...
                logger.warning(f"Subscription {subscription_id} not found for payment {payment_id}")
        ...
        if not Payment.objects.filter(stripe_id=payment_id).exists():
            ...
            logger.info(f"Created payment record: {payment.id}, Stripe ID: {payment_id}")
        else:
...
def handle_payment_intent_failed(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not Payment.objects.filter(stripe_id=payment_id).exists():
            ...
            logger.info(f"Created failed payment record: {payment.id}, Stripe ID: {payment_id}")
        else:
            # Update existing payment to failed status
            Payment.objects.filter(stripe_id=payment_id).update(status=Payment.STATUS_FAILED)
