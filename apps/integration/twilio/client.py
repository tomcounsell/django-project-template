...
class TwilioClient:
    ...
    def _make_request(self, func_name: str, **kwargs) -> Dict[str, Any]:
        ...
        if getattr(settings, 'DEBUG', False) or not self.enabled:
            logger.info(f"[DEBUG/DISABLED] Twilio API request would have been: {func_name}")
    ...
    def send_sms(self, to_number: str, body: str, from_number: Optional[str] = None, 
        ...
        if getattr(settings, 'DEBUG', False) or not self.enabled:
            ...
            return {"success": True, "simulated": True, "sid": "SM00000000000000000000000000000000"}
    ...
    def verify_phone_number(self, phone_number: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.client.lookups.phone_numbers(phone_number).fetch(type="carrier")
            return {
                "success": True,
                "valid": True,
                "data": {
            ...
            if e.code == 20404:  # Invalid phone number
                return {"success": True, "valid": False, "error": "Invalid phone number"}
    ...
    def get_message_status(self, message_sid: str) -> Dict[str, Any]:
        ...
        if getattr(settings, 'DEBUG', False) or not self.enabled:
            logger.info(f"[DEBUG/DISABLED] Would check status of message: {message_sid}")
