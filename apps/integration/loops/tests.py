...
class WaiverEmailTests(TestCase):
    ...
    def test_send_waiver_signing_request(self):
        ...
        with mock.patch('apps.integration.loops.shortcuts.reverse') as mock_reverse:
            with mock.patch('apps.integration.loops.shortcuts.LoopsClient') as MockLoopsClient:
                with mock.patch('apps.integration.loops.client.DEBUG', False):
                    # Set up client mock
                    ...
                    # Check the arguments
                    self.assertEqual(kwargs["transactional_id"], "cm8h2ly3x02kbwes1nz8clc24")
                    self.assertEqual(kwargs["to_email"], "guardian@example.com")
                    self.assertEqual(kwargs["data_variables"]["guardian_name"], "Guardian User")
                    self.assertIn("waiver_link", kwargs["data_variables"])
                    self.assertEqual(kwargs["data_variables"]["child_name"], "Child User")
