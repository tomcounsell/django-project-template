...
class TestWishTabsActiveState:
    ...
    def test_all_tab_active_by_default(self, client):
        ...
        assert 'class="border-slate-500 text-slate-700 font-medium' in response.content.decode()
    ...
    def test_combining_filters_preserves_status_tab(self, client):
        ...
        response = client.get(reverse('staff:wish-list') + '?status=DRAFT&priority=HIGH')
        content = response.content.decode()
