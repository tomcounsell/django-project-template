    ...
    def test_wish_form_input_styling(self, client):
        ...
        for field_name in ["title", "tags"]:
            for style_class in style_classes:
                assert f'class="w-full px-3 py-2 border' in content
                assert style_class in content, f"Style class '{style_class}' missing in {field_name} input"
        
        # Check selects (status, priority, effort, value)
        for field_name in ["status", "priority", "effort", "value"]:
            for style_class in style_classes:
                assert style_class in content, f"Style class '{style_class}' missing in {field_name} select"
        
        # Check textarea (description)
        for style_class in style_classes:
            assert style_class in content, f"Style class '{style_class}' missing in description textarea"
        
        # Check number input (cost_estimate)
        for style_class in style_classes:
            assert style_class in content, f"Style class '{style_class}' missing in cost_estimate input"

    def test_wish_create_modal_styling(self, client):
        """Test the styling of the wish create modal."""
        response = client.get(reverse("staff:wish-create-modal"), HTTP_HX_REQUEST="true")
        assert response.status_code == 200
        ...
        for field, input_type in field_types.items():
            assert f'id="id_{field}"' in content, f"Field {field} not found in modal form"
            
            # Verification depends on modal form implementation
            if input_type == "input" or input_type == "textarea" or input_type == "select":
                for style_class in style_classes:
                    assert style_class in content, f"Style class '{style_class}' missing in {field} {input_type}"