...
class ChatSendMessageView(HTMXView):
    ...
    def process_message_async(
        ...
        try:
            ...
            for msg in chat_session.messages.filter(
                is_processed=True
            ).exclude(
                id=user_message.id
            ).order_by('created_at'):
            ...
            # Handle errors
            assistant_message.content = f"I apologize, but I encountered an error: {str(e)}"
            assistant_message.is_processed = True
            assistant_message.metadata = {
                'status': 'error',
                'error': str(e)
...
class ChatPollMessageView(HTMXView):
    ...
    def get(self, request: HttpRequest, message_id: str, *args, **kwargs) -> HttpResponse:
...
class ChatLoadSessionView(HTMXView):
    ...
    def get(self, request: HttpRequest, session_id: str, *args, **kwargs) -> HttpResponse:
        """Load a chat session."""
