...
def test_todo_item_appearance(driver, live_server, db):
    ...
    assert (
        "w-full" in container_class or "container" in container_class
    ), "Todo container should have proper width class"
    ...
    if todo_items:
        ...
        assert any(
            c in item_class for c in ["p-", "py-", "px-"]
        ), "Todo item should have padding"
        assert any(
            c in item_class for c in ["m-", "my-", "mx-"]
        ), "Todo item should have margin"
        ...
        assert any(
            c in text_class for c in ["text-", "font-"]
        ), "Todo text should have style classes"
...
def test_completed_todo_appearance(driver, live_server, db):
    ...
    TodoItem.objects.create(title="Completed Todo Item", user=user, completed=True)
    ...
    has_completed_style = (
        "line-through" in text_class
        or "completed" in text_class
        or "text-gray" in text_class
    )
