...
async def create_test_scenario(feature_name: str, complexity: str = "basic") -> List[str]:
    """
    ...
    if not BROWSER_USE_AVAILABLE:
        return [f"# browser-use not available - would generate test for: {feature_name}"]
...
async def generate_data_for_model(model_name: str, field_specs: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
    """
    ...
    field_specs_text = ""
    if field_specs:
        field_specs_text = "\n".join([f"- {field}: {spec}" for field, spec in field_specs.items()])
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        data = await generate_data_for_model("User", {"username": "Must be unique", "email": "Must be valid email format"})
        print("\nGenerated data:")
