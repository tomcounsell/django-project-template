...
def test_generate_todo_screenshots():
    ...
    capture_url(f"{server_url}/todos/create/", f"{screenshots_dir}/02_todo_create_form.png")
    ...
    capture_url(f"{server_url}/todos/{todo.id}/", f"{screenshots_dir}/04_todo_detail.png")
    ...
    capture_url(f"{server_url}/todos/{todo.id}/", f"{screenshots_dir}/05_todo_completed_detail.png")
    
    # Step 7: View Todo List with completed item
    response = client.get(reverse('public:todo-list'))
    assert response.status_code == 200
    print(f"Step 7: Viewing todo list with completed item")
    capture_url(f"{server_url}/todos/", f"{screenshots_dir}/06_todo_list_with_completed.png")
