...
class TestAccountSettingsBrowser:
    ...
    async def login_user(self, page, username, password):
        ...
        try:
            ...
            logout_present = await page.locator('form.logout-form, .account-menu, .user-menu').count() > 0
    ...
    async def test_view_account_settings(self, page, test_user):
        ...
        assert await page.locator('input[name="email"]').count() > 0, "Email field not found"
        assert await page.locator('input[name="first_name"]').count() > 0, "First name field not found"
        assert await page.locator('input[name="last_name"]').count() > 0, "Last name field not found"
    ...
    async def test_update_profile(self, page, test_user):
        ...
        success_message = await page.locator('.alert-success, .notification, [role="alert"]').count() > 0
        ...
        assert user.first_name == "Updated", f"User first_name is {user.first_name}, not 'Updated'"
        assert user.last_name == "Name", f"User last_name is {user.last_name}, not 'Name'"
    ...
    async def test_form_validation(self, page, test_user):
        ...
        error_message = await page.locator('.invalid-feedback, .error, [role="alert"]').count() > 0
        ...
        user.refresh_from_db()
        assert user.email == email, f"User email changed to {user.email} despite validation error"