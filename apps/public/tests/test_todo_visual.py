...
async def test_todo_visual_workflow_with_agent():
    ...
    for screenshot in expected_screenshots:
        ...
        assert os.path.exists(
            screenshot_path
        ), f"Expected screenshot {screenshot} was not created"

    print(
        f"Visual test completed successfully with {len(expected_screenshots)} screenshots in {screenshots_dir}"
    )
