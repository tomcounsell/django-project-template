...
def add_team_member(request, team_slug):
    ...
    try:
        ...
        if team.members.filter(id=user.id).exists():
            messages.warning(
                request,
                f'{user.get_full_name()} is already a member of this team.'
            )
            return redirect('public:team-detail', team_slug=team_slug)
        ...
        messages.success(
            request,
            f'{user.get_full_name()} has been added to the team.'
        )
...
def change_member_role(request, team_slug, member_id):
    ...
    if (new_role == Role.OWNER.value or team_member.role == Role.OWNER.value):
        ...
        if not is_owner:
            messages.error(
                request,
                'Only team owners can change owner status.'
            )
            return redirect('public:team-detail', team_slug=team_slug)
...
def remove_team_member(request, team_slug, member_id):
    ...
    if team_member.role == Role.OWNER.value:
        ...
        if not is_owner:
            messages.error(
                request,
                'Only team owners can remove other owners.'
            )
            return redirect('public:team-detail', team_slug=team_slug)
    ...
    messages.success(
        request,
        f'{user_name} has been removed from the team.'
    )
    return redirect('public:team-detail', team_slug=team_slug)
